...
function install() {
    ...
    docker run --restart=always --network=epii-net --ip 172.18.12.99 --name esc-${version} -p $1:80 -p $2:443 -v $a_dir:/epii -itd epii-server:${version} /bin/bash -c "cd /epii-server ; /bin/bash ./start.sh;/bin/bash /epii/server-start.sh;/bin/bash"
    docker exec esc-${version} bash -c "git -C /epii-server remote set-url origin https://gitee.com/epii/epii-server.git"
    docker exec esc-${version} bash -c "git -C /webs/git-auto-website remote set-url origin https://gitee.com/epii/git-auto-website.git"
    pull
    stop
    start
...
function mysql_install() {
    ...
    docker exec -it esc-mysql mysql  -uroot -p$2 -e"USE mysql;alter user 'root'@'localhost'IDENTIFIED BY '$2';CREATE USER 'root'@'172.18.%' IDENTIFIED BY '$2';GRANT all ON *.* TO 'root'@'172.18.%' WITH GRANT OPTION; FLUSH PRIVILEGES;"
